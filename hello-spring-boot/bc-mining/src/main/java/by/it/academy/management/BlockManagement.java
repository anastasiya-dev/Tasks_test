...
public class BlockManagement {
    ...
    private boolean successCheck(BlockTemporary blockTempChecked) {
        logger.info("Success check for block temporary: " + blockTempChecked.getBlockTemporaryId());
    ...
    private MiningSession miningSuccessHandler(String miningSessionId) {
        ...
        miningSession.setBlockIdActual(String.valueOf(block.getBlockId()));
    ...
    private boolean checkIfCurrentBlockAlreadyMined(String blockIdAttempted, BlockTemporary blockTemporary) {
        ...
        if (blockTemporary.getBlockTemporaryId().equals("0_0") ||
                String.valueOf(blockService.findAllBlocks().size()).equals(blockIdAttempted)) {
            logger.info("No");
            return false;
        } else {
            logger.info("Yes");
    ...
    private float sendRewards(MiningSession miningSession) {
        ...
        for (Transaction transactionInPackage : blockTransactions) {
            ...
            transactionService.findTransactionById(transactionInPackage.getTransactionId()).setBlockId(String.valueOf(blockIdAttempted));
