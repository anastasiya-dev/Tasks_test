...
select max(value) from expenses where paydate=(select paydate from (select paydate, sum(value) as daySum from expenses group by paydate) as support where daySum=(select max(daySum) from (select paydate, sum(value) as daySum from expenses group by paydate) as support));

select max(value) from expenses;
select paydate from expenses where value=(select max(value) from expenses);
select sum(value) from expenses where paydate=(select paydate from expenses where value=(select max(value) from expenses));

select e1.paydate from expenses e1 where e1.value=(select max(e2.value) from expenses e2);
